import { defineConfig } from 'vite'
import react from '@vitejs/plugin-react'
import tailwindcss from '@tailwindcss/vite'
import { VitePWA } from 'vite-plugin-pwa';

// https://vite.dev/config/
export default defineConfig({
  plugins: [
    react(),
    tailwindcss(),
    VitePWA({
      registerType: 'autoUpdate',
      manifest: {
        name: "Persan Finance & Docs",
        short_name: "Persan Finance",
        description: "Controle financeiro e documentos familiares",
        start_url: "/",
        display: "standalone",
        theme_color: "rgba(0,0,0,0)",
        background_color: "#F0F9FF",
        display_override: ["window-controls-overlay"],
        icons: [
          // ícones principais
          { src: '/pwa_icons/icon-192x192.png', sizes: '192x192', type: 'image/png' },
          { src: '/pwa_icons/icon-512x512.png', sizes: '512x512', type: 'image/png' },

          // outros tamanhos opcionais (apenas comente/remova se não precisar)
          { src: '/pwa_icons/icon-16x16.png', sizes: '16x16', type: 'image/png' },
          { src: '/pwa_icons/icon-32x32.png', sizes: '32x32', type: 'image/png' },
          { src: '/pwa_icons/icon-48x48.png', sizes: '48x48', type: 'image/png' },
          { src: '/pwa_icons/icon-72x72.png', sizes: '72x72', type: 'image/png' },
          { src: '/pwa_icons/icon-96x96.png', sizes: '96x96', type: 'image/png' },
          { src: '/pwa_icons/icon-120x120.png', sizes: '120x120', type: 'image/png' },
          { src: '/pwa_icons/icon-128x128.png', sizes: '128x128', type: 'image/png' },
          { src: '/pwa_icons/icon-144x144.png', sizes: '144x144', type: 'image/png' },
          { src: '/pwa_icons/icon-152x152.png', sizes: '152x152', type: 'image/png' },
          { src: '/pwa_icons/icon-167x167.png', sizes: '167x167', type: 'image/png' },
          { src: '/pwa_icons/icon-180x180.png', sizes: '180x180', type: 'image/png' },
          { src: '/pwa_icons/icon-384x384.png', sizes: '384x384', type: 'image/png' }
        ]
      },
      workbox: {
        runtimeCaching: [
          {
            urlPattern: ({ request }) => request.destination === 'document',
            handler: 'CacheFirst',
            options: { cacheName: 'documents-cache' }
          },
          {
            urlPattern: ({ url }) => url.origin.includes('api.exchangerate.host'),
            handler: 'NetworkFirst',
            options: { cacheName: 'exchange-cache', expiration: { maxEntries: 30 } }
          }
        ]
      }
    })
  ],
})
